...
執筆中に実行する全ての Gradle タスクは次のとおりです。

[cols="1,2"]
[frame="topbot", grid="none", options="header"]
[caption=""]
.執筆中に実行する Gradle タスク一覧
|======
|タスク名
|役割
|``docs``
...
include 先ファイルの更新や PDF テーマの修正など、``.adoc`` 自体に変更がない場合に ``clean`` タスクを実行後、再ビルドすると結果が反映されます。
...
TIP: Windows PowerShell 環境では、パスの区切り文字を ``\`` ではなく、macOS や Linux と同様に ``/`` とすることでも正しく動作します。環境による操作の差異を省くため、ここからの解説ではパス区切りを ``/`` に表記として示します。また、各ユーティリティタスクが出力するパスも ``/`` 形式に統一しています。

なお、Gradle が持つ全てのタスクは ``tasks`` タスクにより出力でき、文書のビルドで使われるタスクは ``Documentation tasks`` グループに配置されていることが分かります。
...
NOTE: HTML/PDF 変換タスクの結果の出力先は ``build`` フォルダとなっており、``clean`` タスクの実態は ``build`` フォルダ内のファイルを削除する処理です。
...
----
<1> AsciidoctorJ のバージョン指定します。最新バージョンを指定できますが、破壊的変更がある場合にビルドが失敗する場合があるので注意します。
<2> ダイアログ記法を処理する asciidoctor-diagram のバージョン指定です。最新バージョンが指定できます。
