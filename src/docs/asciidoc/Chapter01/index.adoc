...
Windows は標準で WSL2 と呼ばれる Linux 仮想環境を使うことができ、仮想環境上に Ubuntu 22.04 を導入することでも執筆環境の構築が可能です。メインの執筆環境が Ubuntu や macOS の場合に、Windows 上でも WSL2 Ubuntu を使うと各種操作が統一できて便利かもしれません。

WSL2 に Ubuntu 22.04 LTS が導入済みであることを前提に、執筆環境の構築手順は次のようになります。
...
次に、通常の Ubuntu と同様に sdkman で Java 環境の導入を行います。
...
拡張導入後、WSL2 Ubuntu のターミナル画面でカレントディレクトリをプロジェクトフォルダに移し、`code .` コマンドを入力することで VS Code が起動します。
