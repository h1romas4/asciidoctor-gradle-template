...
NOTE: 次項以降 WSL2 環境の手順は特別な記載がある場合を除き Ubuntu 項を参照して操作してください。
...
なお、Ubuntu のファイルシステムは Windows 側からエクスプローラからアクセス可能です。WSL2 上で作成した文書を Windows 側で取得する場合にはこの機能を使うと便利です。
