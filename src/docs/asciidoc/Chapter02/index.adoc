...
文書内の参照リンクは ``.adoc`` 文書中で次のように指定できます。
...
``[sorce]`` ブロックを用いると、文書中にソースコードをシンタックスハイライト付きで埋め込むことができます。
...
本変換ビルドスクリプトでは `asciidoctorj-diagram` が有効になっており、PlantUML 記法を追加のアドイン無しで使うことができます。

==== PlantUML

PlantUML 形式のダイアグラムを記述することで、定義から生成された `.svg` ベクター画像を文書に展開します。

===== シーケンス図の例

[source]
-----
[plantuml, diag-sequence-sample1, svg, pdfwidth=80%, width=80%]
[caption=""]
----
include::diag-sequence-sample1.puml[]
----
-----
...
[source]
-----
[plantuml, diag-sequence-sample2, svg, pdfwidth=30%, width=240px]
[caption=""]
----
include::diag-sequence-sample2.puml[]
----
-----
...
[source]
-----
[plantuml, diag-sequence-sample3, svg, pdfwidth=30%, width=30%]
[caption=""]
----
include::diag-sequence-sample3.puml[]
----
-----
