...
[plantuml, directory-structure-1, svg, pdfwidth=72%, width=768px]
[caption=""]
----
...
[plantuml, directory-structure-2, svg, pdfwidth=55%, width=580px]
[caption=""]
----
...
本手順で VS Code の推奨拡張として導入される Paste Image（``mushan.vscode-paste-image``） は、クリップボード上にある画像をファイルとして指定の位置に格納した上で、Asciidoc 文書びリンクを挿入する便利な拡張です。
...
Asciidoc 文書には次のように画像リンクが挿入されます。また、開いている Asciidoc 文書から ``images/`` として見えるパスに画像ファイルが格納されます。
...
挿入時のファイル名の命名規約などの設定は ``.vscode/setting.json`` で行うことができます。
...
NOTE: Asciidoc 拡張にも同様の機能（menu:AsciiDoc: Paste Image[]）がありますが、現在のところ Paste Image 拡張のほうがファイル名などの設定が柔軟です。VS Code のキーコンフィグをするなどして必要に応じて使い分けると良いでしょう。
...
[plantuml, directory-structure-3, svg, pdfwidth=40%, width=440px]
[caption=""]
----
...
+
* ``./gradlew docs`` の入力は VS Code の統合ターミナルを使うと便利です。
