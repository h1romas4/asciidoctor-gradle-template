...
AsciiDoc の ``[sorce]`` ブロックを用いると、文書中にソースコードをシンタックスハイライト付きで埋め込むことができます。

``[source]`` ブロックは次のように記述します。
...
PlantUML 形式のダイアグラムを記述することで、生成された `.svg` ベクター画像を文書に展開することができます。
...
[source]
-----
[plantuml, diag-sequence-sample3, svg, pdfwidth=40%, width=40%]
[caption=""]
----
