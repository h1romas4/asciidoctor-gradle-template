...
準備ができれば、次の要領で URL を作成しウェブブラウザでアクセスするとクラウド環境が起動します。まずは本手順のリポジトリで試してみるのも良いでしょう。
...
前項の Gitpod と同様に VS Code の拡張は自動導入され、PDF/HTML ビルド用のコンテナ環境も構築されます。

[WARNING]
====
Codespaces 上でビルドを行う場合は、起動時に自動的に行われる統合ターミナル上でのコンテナ構築が終わった後、起動時に開かれた統合ターミナルを再起動し `echo $LANG` が `ja_JP.UTF-8` になっていることを確認した上で `./gradlew docs` コマンドを実行してください。環境が日本語設定になっていない場合、ダイアログ記法の図表の日本語出力が崩れる場合があります。
====

NOTE: Codespaces 向けの設定は `.devcontainer.json` 及び `.vscode/build-codespaces-contaner.sh` ファイルで定義されています。
...
image::2023-09-11-20-13-04.png[]

.Codespaces の場合
https://github.com/codespaces

image::2023-09-11-20-10-35.png[]
...
また、コミット・プッシュ済みに関わらず、翌日執筆を再開する際もこれらの画面から環境をリオープンすると良いでしょう。クラウドのリソース削減になるとともに、VS Code の操作状態が維持されたままになるため便利です。
