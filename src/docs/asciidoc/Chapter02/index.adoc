...
執筆中の文書を、インターネット上に配置した Git リポジトリで管理すると次のようなメリットがあります。

- 修正差分、版数管理ができる。
- 複数環境での執筆、及び共同執筆時にファイルの混乱がなくなる。
- 次項のクラウドでの執筆が可能になる。

ここではインターネット上の Git リポジトリとして GitHub にプロジェクトを格納する手順を示します。
...
ローカルの Git リポジトリの初期化が完了したら、続けて統合ターミナルで前項でクリップボードに保持したコマンドを実行し、GitHub リモートリポジトリーにプロジェクトファイルをプッシュ操作します。
...
VS Code 上で文書の編集を行うと、修正や追加の会ったファイルが左のサイドバーに通知されます。修正が完了したファイルは btn:[+] で Git 上にステージングし、任意のコミットコメントを記載して btn:[Commit] を謳歌することで、Git にコミットされます。
