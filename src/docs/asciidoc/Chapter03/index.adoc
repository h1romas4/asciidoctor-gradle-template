...
. ``.\gradlew.bat asciidoc`` と入力し Gradle ビルドを実行します。初回実行時はビルドに必要なファイルをダウンロードするため少し時間がかかります。次回は数秒で完了します。
...
Asciidoc から変換された HTML/PDF は次の場所に格納されます。ブラウザや PDF ビュアーで表示を確認してみます。

[source]
----
build/asciidoc/html/index.html
