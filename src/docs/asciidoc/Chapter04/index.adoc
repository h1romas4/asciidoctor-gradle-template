...
本手順では PlantUML と ditaa ダイアグラム記法の一部を紹介します。
...
TIP: PlantUML については PDF 出力時に日本語フォントが化けないようビルドで対策が導入されています。
...
TIP: ditta 内で日本語を使う場合は、表示ずれを防ぐため半角スペースなどで文字数を調整する必要があります。

[source]
-----
[ditaa, diag-ditaa-block, svg, shadows=false, separation=true, pdfwidth=70%, width=70%]
[caption=""]
----
    +------------------------+
